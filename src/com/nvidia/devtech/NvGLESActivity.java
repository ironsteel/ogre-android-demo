 ...
 * NvGLESActivity takes the step further by executing the necessary steps in creating a GLES1
 * sample. There are also some useful helper functions created for loading textures and maintaining
 * the render loop, pausing and resuming as necessary.
 */
public abstract class NvGLESActivity extends NvActivity {
	...
	/**
	 * Helper function to load a file into a {@link NvGLESActivity.RawData} object. It'll first try
	 * loading the file from "/data/" and if the file doesn't exist there, it'll try loading it from
	 * the assets directory inside the .APK file. This is to allow the files inside the apk to be
	 * overridden or not be part of the .APK at all during the development phase of the application,
	 * decreasing the size needed to be transmitted to the device between changes to the code.
	 * 
	 * @param filename
	 *            The file to load.
	 * @return The RawData object representing the file's fully loaded data, or null if loading
	 *         failed.
	 */
	...
	/**
	 * Helper function to load a texture file into a {@link NvGLESActivity.RawTexture} object. It'll
	 * first try loading the texture from "/data/" and if the file doesn't exist there, it'll try
	 * loading it from the assets directory inside the .APK file. This is to allow the files inside
	 * the apk to be overridden or not be part of the .APK at all during the development phase of
	 * the application, decreasing the size needed to be transmitted to the device between changes
	 * to the code.
	 * 
	 * The texture data will be flipped and bit-twiddled to fit being loaded directly into OpenGL ES
	 * via the glTexImage2D call.
	 * 
	 * @param filename
	 *            The file to load.
	 * @return The RawTexture object representing the texture's fully loaded data, or null if
	 *         loading failed.
	 */
				...
				if (!init()) {
					handler.post(new Runnable() {
						public void run() {
							new AlertDialog.Builder(act)
									.setMessage(
											"Application initialization failed. The application will exit.")
					...
					painter = new Runnable() {
						...
						public void run() {
							...
							/*
							 * if (fpsText != null && ++count == 100) { long end =
							 * nvGetSystemTime(); fpsText.setText(String.format("%.2f  fps",
							 * (100*1000.0/(end-timeStart)))); timeStart = nvGetSystemTime(); count
							 * = 0; }
							 */
						}
					};
					...
					if (eglInitialized)
						...
						handler.post(new Runnable() {
							public void run() {
								new AlertDialog.Builder(act)
										.setMessage(
												"EGL Initialization failed, the application will exit.")
	...
	/**
	 * Called to initialize EGL. This function should not be called by the inheriting activity, but
	...
	protected boolean initEGL() {
		...
		int score = 1 << 24; // to make sure even worst score is better than this, like 8888 when
								// request 565...
		int val[] = new int[1];
		for (i = 0; i < num_configs[0]; i++) {
			...
			System.out.println(">>> EGL Config [" + i + "] R" + r + "G" + g + "B" + b + "A" + a
					+ " D" + d + "S" + s);

			currScore = (Math.abs(r - redSize) + Math.abs(g - greenSize) + Math.abs(b - blueSize) + Math
					.abs(a - alphaSize)) << 16;
		...
		eglContext = egl
				.eglCreateContext(eglDisplay, eglConfig, EGL10.EGL_NO_CONTEXT, contextAttrs);
	...
	/**
	 * Called to create the EGLSurface to be used for rendering. This function should not be called
	 * by the inheriting activity, but can be overridden if needed.
	 * 
	 * @param surface
	 *            The SurfaceHolder that holds the surface that we are going to render to.
	 * @return True if successful
	 */
	...
	/**
	 * Destroys the EGLSurface used for rendering. This function should not be called by the
	...
	/**
	 * Called to clean up egl. This function should not be called by the inheriting activity, but
	 * can be overridden if needed.
	 */
	...
	/**
	 * When the activity is running, this function will be continuously called to render to the
	 * screen.
	 * 
	 * @param time
	 *            The current time in seconds.
	 * @param drawWidth
	 *            The current width o the render surface.
	 * @param drawHeight
	 *            The current height of the render surface.
	 * @param forceRedraw
	 *            Set to true if the system requests a forced redraw, upon for example changing the
	 *            resolution. If this is set to true the application should render and return true
	 *            from the function to ensure that the framebuffer is swapped. If set to false it's
	 *            up to the implementation to decide whether to render or not.
